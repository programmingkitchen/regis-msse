...
public class FleetRentalManager extends ManagerSuperType
{
	 ...
	 */
			...
			RentalComposite rentalComposite)
	 {
		...
		{
			action =  registerCustomer(IRegistrationService.NAME, rentalComposite);
		}
	...
	/**
	 * Method delegates to the ServiceFactory to execute a service. Good part of this
	 * approach is that the Manager knows the service by a string name - thus achieving
	 * the decoupling effect that we so desire in the MVC approach.
	 *
	 * @param commandString 
	 ...
	 * 			IS THIS WORTH PROPAGATING EXCEPTION TO CALLER(View/Controller), OR SHOULD IT BE HANDLED LIKE BELOW?
	 *          Would propagating exception to the View/Controller in this tie Presentation Layer to Model semantics?
	 *          Is that considered tight coupling? 
	 *
	 */
    public boolean registerCustomer(String commandString, RentalComposite rentalComposite)
    {
    	...
    	try {
			iRegistrationService = (IRegistrationService)serviceFactory.getService(commandString);
    		isRegistered = iRegistrationService.registerCustomer(rentalComposite);
		} catch (ServiceLoadException e1) {
			System.out.println ("FleetRentalManager::registerCustomer failed." + e1.getMessage()); // defaulting to isRegistered to false
    	} catch (RegistrationException e) {
            System.out.println ("FleetRentalManager::registerCustomer failed." + e.getMessage()); // defaulting to isRegistered to false
		}
     ...
     */
	public boolean loginCustomer(String commandString, RentalComposite rentalComposite) 
	{
    	...
    	try {
    		iLoginService = (ILoginService)serviceFactory.getService(commandString);
    		isAuthenticated = iLoginService.authenticateCustomer(rentalComposite);
		} catch (ServiceLoadException e1) {
            System.out.println ("FleetRentalManager::login failed"); // defaulting to isAuthenticated to false
    ...
    public static void main (String[] args)
    {
		...
		Customer customer = new Customer ("Simpson", "Homer", "homer@duff.com", "duff", "619.111.1234","619.111.1234");
