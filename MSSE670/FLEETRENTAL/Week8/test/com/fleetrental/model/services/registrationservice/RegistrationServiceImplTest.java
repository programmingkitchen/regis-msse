...
public class RegistrationServiceImplTest extends TestCase {
	...
	public void setUp() throws Exception {
		...
		try {
			registrationService = (IRegistrationService)serviceFactory.getService("IRegistrationService");
		}
	...
	public final void testRegisterCustomer() {
		...
		try {
			boolean isRegistered = registrationService.registerCustomer(rentalComposite);
	...
	/**
	 * Test for a unsuccessful registration, by checking getting RegistrationException
	 */
	/*
	public final void testRegisterCustomerUnSuccessfulByException() {
		...
		try {
			RentalComposite rentalComposite = new RentalComposite();
			boolean isRegistered = registrationServiceImpl.registerCustomer(rentalComposite);
			assertFalse(isRegistered);
		} catch (RegistrationException e) {
            return;
        }
